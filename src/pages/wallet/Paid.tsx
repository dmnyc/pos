...
export function Paid() {
  ...
  useEffect(() => {
    ...
    // Only show lightning animation if payment effect is enabled
    setShowLightning(config.paymentEffectEnabled);
  ...
  }, [location, config.paymentChimeEnabled, config.paymentEffectEnabled]);
            ...
            <div className="w-full flex flex-col items-center gap-8 md:gap-10 lg:gap-10 lg:landscape:gap-6 wide:gap-12 wide:landscape:gap-8">
              ...
              {showTipButton && (
                <div className="w-full max-w-xs md:max-w-md lg:max-w-md lg:landscape:max-w-sm wide:max-w-xl wide:landscape:max-w-md">
                  {/* TipGlowButton has a 1.5s delay to start after lightning animation completes */}
                  <TipGlowButton
                    onClick={handleTip}
                    theme={config.theme}
